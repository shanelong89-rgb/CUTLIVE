...
- **Manual Supabase migrations** — the following were run directly in the Supabase SQL Editor and are not applied automatically:
  ...
  - `supabase/migrations/normalize_event_districts.sql` — normalizes `district` in `events` and `submissions` to Title Case (slug map + initcap fallback). Submit form now normalizes districts client-side via `normalizeDistrict()` in `Submit.tsx`.
